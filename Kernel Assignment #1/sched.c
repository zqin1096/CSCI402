...
{
	// If the thread is already in cancellation point, return immediately.
	if (curthr->kt_cancelled) {
		...
		sched_switch();
		// If the thread enters cancellation point after it wakes up, return 
		// immediately.
		if (curthr->kt_cancelled) {
			// Self-checks.
			dbg(DBG_PRINT, "(GRADING1C)\n");
...
sched_make_runnable(kthread_t* thr)
{
	...
	uint8_t previous = intr_getipl();
	intr_setipl(IPL_HIGH);
	// Set the thread state to runnable and put it onto the run queue.
	thr->kt_state = KT_RUN;
