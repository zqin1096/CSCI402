...
bootstrap(int arg1, void* arg2)
{
	...
	dbgq(DBG_TEST, "SIGNATURE: 53616c7465645f5f9957a6f20571926bd69a8dfb96a9e5ae52a07f0a33b9448548dc363783d0714bd631d99ca54dfe2c\n");
	/* necessary to finalize page table information */
	pt_template_init();
	// Create the idle process. curproc is defined in proc.c.
	...
	KASSERT(NULL != curthr);
	dbg(DBG_PRINT, "(GRADING1A 1.a)\n");
	// Start running the idle process.
...
{
	// Create the init process and thread.
...
#ifdef __DRIVERS__
int
my_faber_thread_test(kshell_t* kshell, int argc, char** argv)
{
	int status;
	proc_t* faber_proc = proc_create("faber");
	kthread_t* faber_thr = kthread_create(faber_proc, faber_thread_test, 0, NULL);
	sched_make_runnable(faber_thr);
	pid_t child = do_waitpid(faber_proc->p_pid, 0, &status);
	return 0;
}
#endif /* __DRIVERS__ */
...
#ifdef __DRIVERS__
int
my_sunghan_test(kshell_t* kshell, int argc, char** argv)
{
	int status;
	...
	pid_t child = do_waitpid(sunghan_proc->p_pid, 0, &status);
	return 0;
}
#endif /* __DRIVERS__ */
...
#ifdef __DRIVERS__
int
my_sunghan_deadlock_test()
{
	int status;
	proc_t* sunghan_deadlock_proc = proc_create("deadlock");
	...
	pid_t child = do_waitpid(sunghan_deadlock_proc->p_pid, 0, &status);
	return 0;
}
#endif /* __DRIVERS__ */
